...
describe ReferenceCollection do
  ...
  describe "the 'to_s' method" do
    before :each do
      # SMELL: Assumes full book name
      @input_str = 'Genesis 1:1; John 1:1, 3:16-18'
      @rc = BibleReferenceParser.parse(@input_str)
    end

    it 'should render a string' do
      @rc.to_s.should be_kind_of String
    end

    it 'should render a string simular to the original parser input' do
      @rc.to_s.gsub(' ','').downcase.should eql @input_str.gsub(' ','').downcase # "simular" not exact
    end
